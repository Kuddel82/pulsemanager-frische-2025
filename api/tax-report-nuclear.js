/**
 * 🚀 NUCLEAR TAX API - ERWEITERTE MULTI-CHAIN VERSION
 * 
  ...
  do {
    ...
    if (result && result.result && result.result.length > 0) {
      ...
      const transactionsWithMetadata = result.result.map(tx => ({
        ...tx,
        dataSource: 'moralis_nuclear_enhanced',
...
export default async function handler(req, res) {
  console.log('🚀🚀🚀 NUCLEAR TAX API: ERWEITERTE MULTI-CHAIN VERSION - ALLE 5 SCHRITTE AKTIV! 🚀🚀🚀');
  ...
  try {
    ...
    console.log('🚀 NUCLEAR TAX PARAMS:', { 
      chain, 
      address: address ? address.slice(0, 8) + '...' : 'MISSING', 
      limit,
      hasCursor: !!cursor,
      hasDateRange: !!(from_date && to_date),
      status: 'NUCLEAR_ENHANCED_MULTI_CHAIN_VERSION'
    });

    if (!address) {
      return res.status(400).json({ 
        error: 'Missing address parameter.',
        usage: 'POST /api/tax-report-nuclear with address, chain, limit',
        received: params
      });
    }
    ...
    if (allTransactions.length === 0) {
      console.warn(`⚠️ NUCLEAR TAX NO TRANSFER DATA: Returning empty result for ${address}`);
      return res.status(200).json({
        ...
        taxReport: {
          ...
          metadata: {
            source: 'moralis_nuclear_enhanced_empty',
    ...
    console.log(`✅ NUCLEAR TAX TRANSFERS LOADED: ${categorizedTransactions.length} transfers for ${address}, categorized for tax reporting`);
    ...
    const summary = {
      ...
      status: "NUCLEAR_ENHANCED_COMPLETE_VERSION_ALL_TRANSACTION_TYPES"
    };
    ...
    return res.status(200).json({
      ...
      taxReport: {
        ...
        summary: summary,
        metadata: {
          source: 'moralis_nuclear_enhanced_success',
          chains: chains.map(c => c.name),
          address: address,
          timestamp: new Date().toISOString(),
          count: categorizedTransactions.length,
          status: 'NUCLEAR_ENHANCED_COMPLETE_VERSION',
    ...
    console.error('💥 NUCLEAR TAX API CRITICAL ERROR:', error);
    ...
    return res.status(200).json({
      ...
      taxReport: {
        ...
        metadata: {
          source: 'moralis_nuclear_enhanced_error',
          error: error.message,
          timestamp: new Date().toISOString(),
          debug: 'Nuclear enhanced multi-chain version error'
        }
      }
    });
  }
