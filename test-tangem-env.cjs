// 🚀 TANGEM WALLET TEST mit Environment Variables (CommonJS)
...
async function testTangemWallet() {
  try {
    ...
    if (tokens.length > 0) {
      ...
      try {
        ...
        if (priceResponse.ok) {
          ...
          if (price.usdPrice > 0) {
            const balance = parseFloat(testToken.balance) / Math.pow(10, testToken.decimals || 18);
    ...
    console.log('🚀 Jetzt können Sie die Webanwendung unter http://localhost:5173/ testen!');
